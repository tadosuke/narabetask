...
describe('TaskStaging', () => {
  ...
  it('should show empty state when no unplaced tasks exist', () => {
    ...
    expect(screen.getByText('タスクがありません。', { exact: false })).toBeInTheDocument();
    expect(screen.getByText('「新しいタスク」ボタンでタスクを作成してください。', { exact: false })).toBeInTheDocument();
  ...
  it('should render instructions section', () => {
    ...
    expect(screen.getByText('タスクをクリックして設定を編集')).toBeInTheDocument();
    expect(screen.getByText('タスクをドラッグしてタイムラインに配置')).toBeInTheDocument();
    expect(screen.getByText('配置したタスクは重複できません')).toBeInTheDocument();
  });

  it('should have correct CSS classes', () => {
    render(<TaskStaging {...defaultProps} />);
    
    const stagingContainer = screen.getByText('タスク一覧').closest('.task-staging');
